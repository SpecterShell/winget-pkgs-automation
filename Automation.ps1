...
Param (
    [Parameter(
        Mandatory = $false,
        HelpMessage = "Used for updating/debugging a specific package's formula.",
        DontShow = $true # Hide this parameter from the help message
    )]
...
# Fetch latest version of WinGet Automation CLI
Write-Output 'Downloading latest version of WinGet Automation CLI...'
switch -regex ($Env:OS) {
    'Windows_NT' { Set-Variable -Name cliOS -Value 'windows' -Force }
    'Linux' { Set-Variable -Name cliOS -Value 'linux' -Force }
    'Darwin' { Set-Variable -Name cliOS -Value 'darwin' -Force }
    Default { Write-Error "Unsupported OS: $Env:OS"; Exit 1 }
}
...
Write-Output "OS: $cliOS, Architecture: $cliArch"
Invoke-WebRequest -Uri https://github.com/vedantmgoyal2009/winget-pkgs-automation/releases/download/nightly/winget-pkgs-automation_$($cliOS)_$($cliArch).exe -OutFile .\winget-pkgs-automation.exe
Write-Output 'Downloaded latest version of WinGet Automation CLI.'

# Get package json when package identifier is provided through parameter
If ($MyInvocation.BoundParameters.ContainsKey('FormulaPath')) {
    $PkgFromParam = @(Get-Content -Path $FormulaPath -Raw | ConvertFrom-Json)
}

Write-Output 'Checking for updates...'
$ErrorGettingUpdates = @()
$ErrorUpgradingPkgs = New-Object -TypeName System.Collections.ArrayList
ForEach ($Package in $PkgFromParam ?? (Get-ChildItem .\Formula\ -Recurse -File | Get-Content -Raw | ConvertFrom-Json)) {
    ...
    try {
        for ($_Index = 0; $_Index -lt $Package.Update.Length; $_Index++) {
            ...
            If (-not [System.String]::IsNullOrEmpty($Package.Update[$_Index].Headers)) {
                $Package.Update[$_Index].Headers.PSObject.Properties | ForEach-Object `
                    -Begin { $Headers = @{} } `
                    -Process { ($_.Value -eq '$AuthToken' -and $env:GITHUB_ACTIONS -eq 'true') ? $Headers.Add($_.Name, "Bearer $(Get-GitHubBotToken)") : $Headers.Add($_.Name, $_.Value) } `
    ...
    If ($MyInvocation.BoundParameters.ContainsKey('FormulaPath') -and $env:GITHUB_ACTIONS -ne 'true') {
        ...
        ConvertTo-Json -InputObject $Package -Depth 7 | Set-Content -Path .\Formula\$($Package.Identifier.ToLower()[0])\$($Package.Identifier.ToLower()).json
        ...
        try {
            ...
            #     }
            # }
            $CmdToRun = ".\winget-pkgs-automation update '{0}' '{1}' --urls '{2}' --submit" -f $Package.Identifier, $Update.PackageVersion, ($Update.InstallerUrls -join ',')
            If ($Update.AdditionalMetadata) {
                $CmdToRun += " --more-info-json '$(ConvertTo-Json ($Update.AdditionalMetadata) -Compress -Depth 7)'"
            }
            Write-Output "Running command: $CmdToRun"
            $CmdToRun | Invoke-Expression *>&1 | Tee-Object -Variable CliExecStdOut
        } catch {
            Write-Error "$($Package.Identifier): $($_.Exception.Message)"
            $ErrorUpgradingPkgs.Add("- **$($Package.Identifier)**: $($_.Exception.Message)`n```````n$($CliExecStdOut)`n```````n")
            # Revert the changes in the JSON file so that the package can check for updates in the next run
            git checkout -- .\Formula\$($Package.Identifier.ToLower()[0])\$($Package.Identifier.ToLower()).json
        } finally { Write-Output '::endgroup::' }
    } Else {
        Write-Output "[$($Package.Identifier)] No updates available."
    }
    If ($_SkipPackage) {
        $_Package = Get-Content -Path .\Formula\$($Package.Identifier.ToLower()[0])\$($Package.Identifier.ToLower()).json -Raw | ConvertFrom-Json
        $_Package.SkipPackage = 'Automatically marked as stale, not updated for 2.5 years'
        ConvertTo-Json -InputObject $_Package -Depth 7 | Set-Content -Path .\Formula\$($Package.Identifier.Substring(0,1).ToLower())\$($Package.Identifier.ToLower()).json
        Move-Item -Path .\Formula\$($Package.Identifier.ToLower()[0])\$($Package.Identifier.ToLower()).json -Destination .\Formula-skipped\$($Package.Identifier.ToLower()).json -Force
    }
    for ($i = 0; $i -lt $Package.Update.Length; $i++) {
        Remove-Variable -Name "Response$($i -ge 1 ? $i + 1: $Null)" -ErrorAction SilentlyContinue
    }
    Remove-Variable -Name UpdateCondition, CliExecStdOut, _Package, _SkipPackage -ErrorAction SilentlyContinue
}

# Return from here, since the rest of the script is not meant to be run on
## local machine, and only on GitHub Actions runner
If ($env:GITHUB_ACTIONS -ne 'true') { Exit 0 }
...
$CommentBody = @"
### Results of Automation run [$env:GITHUB_RUN_NUMBER](https://github.com/vedantmgoyal2009/winget-pkgs-automation/actions/runs/$env:GITHUB_RUN_ID)
...
(Invoke-RestMethod -Method Get -Uri 'https://api.github.com/repos/vedantmgoyal2009/winget-pkgs-automation/issues/900/comments').Where({
...
Invoke-RestMethod -Method Post -Uri 'https://api.github.com/repos/vedantmgoyal2009/winget-pkgs-automation/issues/900/comments' -Body (ConvertTo-Json -InputObject @{
        body = $CommentBody
    }) -Headers $Headers
...
git add .\Formula\* .\Formula-skipped\*
git commit -m "build(wpa): update packages [$env:GITHUB_RUN_NUMBER] [skip ci]" --signoff
git push https://x-access-token:$(Get-GitHubBotToken)@github.com/vedantmgoyal2009/winget-pkgs-automation.git
